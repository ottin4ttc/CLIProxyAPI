...
func TestHomeRetryPolicyAllowsRemoteCooldownWithoutLocalCredentials(t *testing.T) {
	...
	wait, shouldRetry := manager.shouldRetryAfterError(errRemoteCooldown, 0, []string{"home-retry-contract"}, "gpt", time.Second, false)
	if !shouldRetry || wait != 10*time.Millisecond {
		t.Fatalf("shouldRetryAfterError(, false) = (%v, %t), want (10ms, true)", wait, shouldRetry)
	}
	if _, shouldRetry = manager.shouldRetryAfterError(errRemoteCooldown, 1, []string{"home-retry-contract"}, "gpt", time.Second, false); shouldRetry {
		t.Fatal("shouldRetryAfterError(, false) retried after the configured Home retry round")
	}
	wait, shouldRetry = manager.shouldRetryAfterError(errRemoteCooldown, 0, []string{"home-retry-contract"}, "gpt", 0, false)
	if shouldRetry || wait != 0 {
		t.Fatalf("shouldRetryAfterError(, false) with zero wait interval = (%v, %t), want (0, false)", wait, shouldRetry)
	}
	...
	wait, shouldRetry = manager.shouldRetryAfterError(errRoundExhausted, 0, []string{"home-retry-contract"}, "gpt", 0, false)
	if !shouldRetry || wait != 0 {
		t.Fatalf("shouldRetryAfterError(, false) immediate round = (%v, %t), want (0, true)", wait, shouldRetry)
	}
	...
	if wait, shouldRetry = manager.shouldRetryAfterError(errInvalidWait, 0, []string{"home-retry-contract"}, "gpt", 0, false); shouldRetry || wait != 0 {
		t.Fatalf("shouldRetryAfterError(, false) negative wait = (%v, %t), want (0, false)", wait, shouldRetry)
	}
}
...
func TestRetryIntervalFiltersCooldownCredentials(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			wait, shouldRetry := manager.shouldRetryAfterError(&Error{HTTPStatus: http.StatusTooManyRequests}, 0, []string{provider}, model, 30*time.Second, false)
			if shouldRetry != test.wantRetry {
				t.Fatalf("shouldRetryAfterError(, false) = (%v, %t), want retry %t", wait, shouldRetry, test.wantRetry)
			}
			if test.wantRetry && (wait <= 0 || wait > test.maxWantWait) {
				t.Fatalf("shouldRetryAfterError(, false) wait = %v, want the earliest cooldown within %v", wait, test.maxWantWait)
			}
		})
	}
}
...
func TestHomeRetryPolicyUsesRemoteCredentialOverrideBeforeSelection(t *testing.T) {
	...
	wait, shouldRetry := manager.shouldRetryAfterErrorWithHomeRetryLimit(context.Background(), cliproxyexecutor.Options{}, errRemoteCooldown, 0, []string{"home-retry-contract"}, "gpt", time.Second, -1, 0, false)
	if !shouldRetry || wait != 10*time.Millisecond {
		t.Fatalf("remote credential override retry = (%v, %t), want (10ms, true)", wait, shouldRetry)
	}
	if _, shouldRetry = manager.shouldRetryAfterErrorWithHomeRetryLimit(context.Background(), cliproxyexecutor.Options{}, errRemoteCooldown, 1, []string{"home-retry-contract"}, "gpt", time.Second, -1, 0, false); shouldRetry {
	...
	if _, shouldRetry = manager.shouldRetryAfterErrorWithHomeRetryLimit(context.Background(), pinnedOpts, errRemoteCooldown, 0, []string{"home-retry-contract"}, "gpt", time.Second, -1, 0, false); shouldRetry {
	...
	if _, shouldRetry = manager.shouldRetryAfterErrorWithHomeRetryLimit(context.Background(), cliproxyexecutor.Options{}, errRemoteCooldown, 0, []string{"home-retry-contract"}, "gpt", time.Second, -1, 0, false); shouldRetry {
...
func TestHomeRetryRoundCredentialLimitStartsNextRoundImmediately(t *testing.T) {
	...
	wait, shouldRetry := manager.shouldRetryAfterError(errRoundExhausted, 0, []string{"home-retry-contract"}, "gpt", time.Second, false)
	if !shouldRetry || wait != 0 {
...
func TestHomeCooldownClassificationPreservesNonRetryableRoundStatus(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if wait, shouldRetry := manager.shouldRetryAfterErrorWithHomeRetryLimit(context.Background(), cliproxyexecutor.Options{}, errExecute, 0, []string{"home-retry-contract"}, "gpt", time.Second, retryLimit, 0, false); shouldRetry || wait != 0 {
...
func TestHomeRetryRoundStartsImmediatelyWhenHomeReportsAvailableNextRound(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			wait, shouldRetry := manager.shouldRetryAfterErrorWithHomeRetryLimit(context.Background(), cliproxyexecutor.Options{}, errExecute, 0, []string{"home-retry-contract"}, "gpt", 10*time.Second, retryLimit, 0, false)
			if !shouldRetry || wait != 0 {
