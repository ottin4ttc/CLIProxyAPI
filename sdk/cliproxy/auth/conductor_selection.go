...
func (m *Manager) availableAuthsForRouteModelWithPriorityMode(auths []*Auth, provider, routeModel string, now time.Time, allPriorities bool) ([]*Auth, error) {
	...
	if len(availableByPriority) == 0 {
		...
		if cooldownCount == len(auths) && !earliest.IsZero() {
			...
			return nil, newModelCooldownErrorFull(routeModel, providerForError, resetIn, cooldownReasonForModel(auths, routeModel), lastCandidateErr)
		}
...
func restoreModelCooldownErrorModel(err error, requestedModel string) error {
	...
	return newModelCooldownErrorFull(requestedModel, cooldownErr.provider, cooldownErr.resetIn, cooldownErr.reason, cooldownErr.cause)
}
...
func (m *Manager) shouldRetryAfterError(err error, attempt int, providers []string, model string, maxWait time.Duration, fallbackEligible bool) (time.Duration, bool) {
	...
	return m.shouldRetryAfterErrorWithHomeRetryLimit(context.Background(), cliproxyexecutor.Options{}, err, attempt, providers, model, maxWait, -1, defaultRequestRetry, fallbackEligible)
}
...
func (m *Manager) shouldRetryAfterErrorWithHomeRetryLimit(ctx context.Context, opts cliproxyexecutor.Options, err error, attempt int, providers []string, model string, maxWait time.Duration, homeRetryLimit int, defaultRequestRetry int, fallbackEligible bool) (time.Duration, bool) {
	return m.shouldRetryAfterErrorFull(ctx, opts, err, attempt, providers, model, maxWait, homeRetryLimit, defaultRequestRetry, nil, fallbackEligible)
}
...
func (m *Manager) shouldRetryAfterErrorWithAttempted(ctx context.Context, opts cliproxyexecutor.Options, err error, attempt int, providers []string, model string, maxWait time.Duration, homeRetryLimit int, defaultRequestRetry int, attempted map[string]struct{}) (time.Duration, bool) {
	return m.shouldRetryAfterErrorFull(ctx, opts, err, attempt, providers, model, maxWait, homeRetryLimit, defaultRequestRetry, attempted, false)
}

// shouldRetryAfterErrorFull is the shared implementation. attempted holds the
// credentials tried in the round that just failed so a 429 round never retries
// them with zero wait; fallbackEligible caps overload rotation for requests
// that can fall back to a lower Codex tier instead.
func (m *Manager) shouldRetryAfterErrorFull(ctx context.Context, opts cliproxyexecutor.Options, err error, attempt int, providers []string, model string, maxWait time.Duration, homeRetryLimit int, defaultRequestRetry int, attempted map[string]struct{}, fallbackEligible bool) (time.Duration, bool) {
	if err == nil {
		return 0, false
	}
