...
type modelCooldownError struct {
	...
	reason string
	// cause carries the latest upstream error observed across the candidates.
	cause error
}
...
func NewModelCooldownError(model, provider string, resetIn time.Duration) error {
	return newModelCooldownErrorFull(model, provider, resetIn, "", nil)
}
...
func newModelCooldownErrorWithCause(model, provider string, resetIn time.Duration, cause error) *modelCooldownError {
	return newModelCooldownErrorFull(model, provider, resetIn, "", cause)
}

// newModelCooldownErrorFull builds the error with both the agreed cooldown reason
// (used to gate codex tier fallback) and the latest upstream cause (used for diagnostics).
func newModelCooldownErrorFull(model, provider string, resetIn time.Duration, reason string, cause error) *modelCooldownError {
	if resetIn < 0 {
		resetIn = 0
	}
...
func getAvailableAuthsWithPriorityMode(auths []*Auth, provider, model string, now time.Time, allPriorities bool) ([]*Auth, error) {
	...
	if len(availableByPriority) == 0 {
		if cooldownCount == len(auths) && !earliest.IsZero() {
			...
			return nil, newModelCooldownErrorFull(model, providerForError, resetIn, cooldownReasonForModel(auths, model), nil)
		}
