...
func (s *authScheduler) mixedUnavailableErrorLocked(providers []string, model string, predicate func(*scheduledAuth) bool) error {
	...
	for _, providerKey := range providers {
		...
		localTotal, localCooldownCount, localUnauthorizedCount, localEarliest, _ := shard.availabilitySummaryLocked(predicate)
		total += localTotal
	...
	if cooldownCount == total && !earliest.IsZero() {
		...
		return newModelCooldownErrorFull(model, "", resetIn, "", lastCandidateErr)
	}
...
func (m *modelScheduler) unavailableErrorLocked(provider, model string, predicate func(*scheduledAuth) bool) error {
	now := time.Now()
	total, cooldownCount, unauthorizedCount, earliest, cooling := m.availabilitySummaryLocked(predicate)
	...
	if cooldownCount == total && !earliest.IsZero() {
		...
		return newModelCooldownErrorFull(model, providerForError, resetIn, cooldownReasonForModel(cooling, model), lastCandidateErr)
	}
	if unauthorizedCount == total {
...
// availabilitySummaryLocked summarizes total candidates, cooldown count, unauthorized count,
// earliest retry time, and the *Auth for every entry counted in cooldownCount (i.e. matching
// predicate and currently cooling).
func (m *modelScheduler) availabilitySummaryLocked(predicate func(*scheduledAuth) bool) (total int, cooldownCount int, unauthorizedCount int, earliest time.Time, cooling []*Auth) {
	if m == nil {
	...
	return total, cooldownCount, unauthorizedCount, earliest, cooling
}
