...
func (m *Manager) Execute(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (cliproxyexecutor.Response, error) {
	...
	fallbackEligible := m.codexFallbackEligible(normalized, retryModel, opts)
	for attempt := 0; ; attempt++ {
		...
		lastErr = errExec
		wait, shouldRetry := m.shouldRetryAfterErrorFull(ctx, opts, errExec, attempt, normalized, retryModel, maxWait, -1, defaultRequestRetry, roundAttempted, fallbackEligible)
		if !shouldRetry {
			break
		}
...
func (m *Manager) ExecuteStream(ctx context.Context, providers []string, req cliproxyexecutor.Request, opts cliproxyexecutor.Options) (*cliproxyexecutor.StreamResult, error) {
	...
	for {
		...
		lastErr = errStream
		wait, shouldRetry := m.shouldRetryAfterErrorFull(ctx, opts, errStream, attempt, normalized, retryModel, maxWait, homeRetryLimit, defaultRequestRetry, roundAttempted, fallbackEligible)
		if !shouldRetry {
			break
		}
		if errWait := waitForCooldown(ctx, wait, maxWait); errWait != nil {
