...
// cooldownStateRecordEqual compares the cooldown-carrying fields of two records.
// UpdatedAt is deliberately excluded: MarkResult bumps auth.UpdatedAt on every
// result to keep Generation/UpdatedAt monotonic for scheduler staleness checks,
// so including it here would mark every successful request as a cooldown-state
// change and rewrite the persisted state on each request.
