	...
	if buffering {
		for {
			...
			if wsErr, ok := parseCodexWebsocketError(payload); ok {
				// parseCodexWebsocketError classifies status-less overload frames as
				// retryable 429s, so they land here before the terminal-failure branch
				// below. The conductor retries them on another credential, so the
				// downstream session must not be told the upstream went away.
				failoverPending := isCodexOverloadBootstrapFailure(payload)
				if sess != nil {
					if failoverPending {
