...
func TestCodexExecutor_BootstrapBuffering_OverloadFailsAttemptWithoutLeakingHandshake(t *testing.T) {
	...
	// This fork maps every in-band overload rejection to a retryable 429 (newCodexStatusErr),
	// buffered or not, so the overload cooldown ladder sees one status.
	if got := statusCodeFromTestError(t, err); got != http.StatusTooManyRequests {
		t.Fatalf("status code = %d, want %d (upstream hides the rejection behind HTTP 200)", got, http.StatusTooManyRequests)
	}
}
...
func TestCodexExecutor_BootstrapBuffering_NonContentFramesDoNotReleaseStream(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// This fork maps every in-band overload rejection to a retryable 429 (newCodexStatusErr),
			// so the buffered failover surfaces as 429 rather than upstream's 503.
			if got := statusCodeFromTestError(t, err); got != http.StatusTooManyRequests {
				t.Fatalf("status code = %d, want %d", got, http.StatusTooManyRequests)
			}
		})
...
func TestCodexExecutor_BootstrapBuffering_DefaultDisabledPassthrough(t *testing.T) {
	...
	// included: this fork already maps an unbuffered overload to a retryable 429.
	if got := statusCodeFromTestError(t, streamErr); got != http.StatusTooManyRequests {
		t.Fatalf("status code = %d, want %d while buffering is disabled", got, http.StatusTooManyRequests)
	}
}
